...
The following laws describe the specifics of the united soccer event. For general specifications relevant to all HuroCup events (e.g., robot dimensions, playing field and lighting, responsibility of the referees) please refer to [General - HuroCup Laws of the Game](https://docs.google.com/document/d/15laUlB6uZ56J5WpGPhepb7P8O7ul-8K5sgdf4uwu4Ak/pub).
...
|Dimension|Label|Kid Size|Adult Size|
|:---|:---|:---|
|Total Length|$L|660cm - 760cm|660cm - 760cm|
|Total Height|$W|510cm - 610cm|510cm - 610cm|
|Border Width and Height|$BW|> 30cm|> 30cm|
|Field line width|$LW|4.5cm - 5.5cm|4.5cm - 5.5cm|
|Goal Width|$GW|200cm|200cm|
|Goal Length|$GL|30cm|30cm|
|Goal Height|$GH|70cm|70cm|
|Goalbox Width|$DW|250cm|250cm|
|Goalbox Length|$DL|50cm|50cm|
|Penaltybox Width|$PW|300cm|300cm|
|Penaltybox Length|$PL|120cm|120cm|
|Penalty Mark Length|$PM|100cm|100cm|
|Centre Circle Diameter|$DC|100cm|100cm|
|Penalty Arc Radius|$PC|50cm|50cm|
...
<h3 id="3.7">[US-4]: Number of Robots</h3>
...
[US-9.7]: A goal can not be scored directly from a kick-off. Either the ball must be moved by at least 20 cm or be touched by another player before it is being kicked into the goal. If a team violates this rule, then a kick-off is awarded to the opposing team.[[1]](#21)
<h3 id="10">[US-10]: Dropped Ball</h3>
...
|:---|:---|
|Action|Points|
|Scoring a goal by a teammate|+2|
|Scoring a goal by the opposing team|-2|
|Scoring a goal|+10|
|Making a save to prevent a goal|+7|
|Kicking the ball at the passing pole|+2|
|Removal of the player for a 30 sec. penalty|-1|
...
###[US-20]: Tiebreaker
<p id="20.1">[US-20.1]:</p> In case of a tie between two robots with the same number of points, the number of goals scored will be used as a tiebreaker.

<p id="20.2">[US-20.2]:</p> If there is still a tie after applying rule [US-20.1](#20.1), the robot with the maximum number after removal of the negative points scored will be declared the winner.

[US-20.3]: If there is still a tie after applying rule [US-20.2](#20.2), the winner will be determined via a penalty kick of five rounds, which may be continued via a sudden death penalty kick shootout if necessary.
...
|ID|UINT8|Robot or referee ID|
|Sequence Number|UINT16|Robots must increment this for each message sent|
|Number of Objects NUM|UINT8|Number of objects in the message|
|Objects|OBJECTS[NUM]|The objects describing the state, perceptions, and intentions of the robot|
|Dummy|UINT8|Future extensions, 0x00 for now|
|Checksum|UINT8|Sum of all the bytes in the message|
...
|Name|Type|Comment|
|:---|:---|:---|
|Type|UINT8|0=UNDEFINED<br>1=Own position<br>2=Ball<br>3=Team robot<br>4=Opponent robot <br>5=Team goal<br>6=Opponent goal|
|Location X|UINT16|X position of the object in mm|
|Location Y|UINT16|Y position of the object in mm|
|Orientation|INT16|Orientation in RAD1000|
|Confidence|UINT8|Confidence level|

<u>Perception Object</u>
...
|Name|Type|Comment|
|:---|:---|:---|
|Type|UINT8|32=UNDEFINED<br>33=Own position<br>34=Ball35=Team robot<br>36=Opponent robot<br>37=Team goal<br>38=Opponent goal<br>39=Field line<br>40=Center circle arc|
|Location X|UINT16|X Location of the object in mm|
|Location Y|UINT16|Y Location of the object in mm|
|Orientation|INT16|Orientation of the object in RAD1000|
|Confidence|UINT8|Confidence level|

<u>Intention Object</u>

<u>[US-21.19]:</u> The following shows the format of the intention object.
...
|Type|UINT8|64=UNDEFINED<br>65=Shot on goal<br>66=Play goal keeper<br>67=Movement|
|Location X|UINT16|X target location in mm|
|Location Y|UINT16|Y target location in mm|
|Orientation|INT16|Target orientation in RAD1000|
|Offence|UINT8|Aggressiveness of the intention|

<u>Referee Objects</u>

<u>[US-21.21]:</u> A controller connected to the same wireless network will transmit messages from the referee to the players.
...
|Name|Type|Comment|
|:---|:---|:---|
|Type|UINT8|96=UNDEFINED<br>97=Game State|
|Team Left|UINT16|Bitmask of players from the left team that are allowed to play|
|Team Right|UINT16|Bitmask of the players from the right team that are allowed to play|
|Time|INT16|Remaining seconds in the period of play|
|State|UINT8|0=Game stopped<br>1=Set: Kick off Left Team<br>2=Set: Kick off Right Team<br>3=Play: Kick off Left Team<br>4=Play: Kick off Right Team<br>5=Play: Ball in play|

<u id="21">Extension Objects

[US-21.24]:</u> A team may use extension objects to transmit additional information as long as it does not violate the bandwidth and transfer rate limits described in [US-3.7](#3.7)

[US-21.25]: If desired, multiple extension objects can be transmitted in a message to transmit larger proprietary data.

|Name|Type|Comment|
|:---|:---|:---|
|Type|UINT8|128=User defined<br>129 .. 255|
